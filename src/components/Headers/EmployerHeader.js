...
import "./header.css";
import EmployerProjects from "../projectsBlock/EmployerProjects";

import { Button, Container } from "reactstrap";

function EmployerHeader(props) {
  let pageHeader = React.createRef();
  const [projectDetails, setProjectsDetails] = useState(props.data);
  React.useEffect(() => {
  ...
  return (
    <>
      ...
      >
        <div
          className="employerHeader_content"
          style={{
            marginLeft: "0px",
            marginRight: "0px",
            zIndex: "10000",
        ...
        >
          <h1>Employee's Currently working on Projects</h1>
          <br />
          <div
            style={{ display: "flex", flexWrap: "wrap", alignItems: "center" }}
          >
            {projectDetails.map((details, index) => {
              return <EmployerProjects key={index} details={details} />;
            })}
          </div>
        </div>
      </div>
